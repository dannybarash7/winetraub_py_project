...
from OCT2Hist_UseModel.utils.gray_level_rescale import gray_level_rescale_v2
from OCT2Hist_UseModel.utils.masking import mask_gel_and_low_signal
from zero_shot_segmentation.consts import CROP_HISTOLOGY, GEL_BOTTOM_ROW, APPLY_MASKING, \
...
def crop_oct_from_trapezoid(oct_image):
    ...
    (bottom_left_x, bottom_left_y), (bottom_right_x, bottom_right_y) = calculate_bottom_corners(height, top_left,
...
def predict_oct(oct_input_image_path, mask_true, weights_path, args, create_vhist=True, output_vhist_path=None,
                prompts=None, dont_care_mask=None, vhist_path=None, bcc_mask_true=None, bcc2_mask_true=None):
    # Load OCT image
    ...
    crop_args, cropped_dont_care_mask, cropped_histology_gt, cropped_oct_unscaled, scaled_cropped_oct_without_gel, cropped_bcc_mask_true, cropped_bcc2_mask_true = preprocess_oct(
        dont_care_mask, oct_image,
        mask_true, bcc_mask_true, bcc2_mask_true)

    if create_vhist:
        if vhist_path is not None and not os.path.exists(vhist_path):
        ...
        if cropped_bcc2_mask_true is not None and cropped_bcc2_mask_true.any():
            cropped_bcc2_mask_true = crop_mask_to_non_black_values(cropped_bcc2_mask_true, virtual_histology_image)

        if SHRINK_BCC:
            cropped_bcc_mask_true = crop_mask_x_percent(cropped_bcc_mask_true, percent=10)
            raise Exception("bcc2")
        ...
        if SEGMENT_EPIDERMIS:
            segmentation, points_used, prompts = run_gui_segmentation(virtual_histology_image, weights_path,
        ...
        if bcc2_mask_true is not None:
            # segment the bcc2

            bcc2_segmentation, points_used, prompts = run_gui_segmentation(virtual_histology_image, weights_path,
                                                                           gt_mask=cropped_bcc2_mask_true, args=args,
                                                                           prompts=prompts,
                                                                           dont_care_mask=cropped_dont_care_mask)
        else:
            bcc2_segmentation = None
        if not SEGMENT_EPIDERMIS:
            segmentation = [np.zeros_like(virtual_histology_image[:, :, 0])]
            points_used, prompts = None, prompts
        ...
        if bcc2_mask_true is not None:
            ...
            bcc2_segmentation, points_used, prompts = run_gui_segmentation(scaled_cropped_oct_without_gel, weights_path,
                                                                           gt_mask=cropped_bcc2_mask_true, args=args,
                                                                           prompts=prompts,
                                                                           dont_care_mask=cropped_dont_care_mask)
        else:
    ...
    return (segmentation, virtual_histology_image, cropped_histology_gt, cropped_oct_unscaled, points_used, mask_true,
            prompts, crop_args, scaled_cropped_oct_without_gel, bcc_segmentation, cropped_bcc_mask_true,
            bcc2_segmentation, cropped_bcc2_mask_true)
...
def predict_histology(oct_input_image_path, mask_true, weights_path, args, create_vhist=True, output_vhist_path=None,
    ...
    # for good input points, we need the gel masked out.
    if CROP_HISTOLOGY:
        crop_args, cropped_dont_care_mask, cropped_histology_gt, cropped_histology = preprocess_histology(
            dont_care_mask, histology_image,
            warped_mask_true)
    else:
    ...
    segmentation, points_used, prompts = run_gui_segmentation(cropped_histology, weights_path,
                                                              gt_mask=cropped_histology_gt, args=args, prompts=prompts,
                                                              dont_care_mask=cropped_dont_care_mask)
    virtual_histology_image = None
    # bounding_rectangle = utils.bounding_rectangle(cropped_histology_gt)
...
def preprocess_oct(dont_care_mask, oct_image, warped_mask_true, bcc_mask_true, bcc2_mask_true):
    ...
    cropped_bcc2_mask_true = crop(bcc2_mask_true, **crop_args)

    return crop_args, cropped_dont_care_mask, cropped_histology_gt, cropped_oct_unscaled, scaled_cropped_oct_without_gel, cropped_bcc_mask_true, cropped_bcc2_mask_true
